...
export default function Posts() {
  ...
  // Handle saving the post (either creating or updating)
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        if (selectedPost) {
          // Update the post list when editing
          setPosts(
            posts.map((post) =>
          ...
          // Add new post to the list when creating
  ...
  async function handleDelete(postId) {
    ...
    if (!confirmed) return; // Stop if the user doesn't confirm
    ...
    try {
      ...
      if (res.ok) {
        // Remove the deleted post from the local state without reloading the page
    ...
    <div>
      ...
      <button
        onClick={() => {
          ...
          setSelectedPost(null); // Clear selected post (in case we had an edit mode before)
          setFormData({ title: "", body: "" }); // Reset form data
        }}
                ...
                <td className="px-4 py-2 flex space-x-2">
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      handleDelete(post.id); // Pass the post ID directly
                    }}
      ...
      {/* Modal for editing or creating post */}
      {isModalOpen && (
